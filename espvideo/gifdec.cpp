...
gd_open_gif(const char *fname)
{
    ...
    // XFM: transform 24bits GCT RGB 888 to 16bits 565
    for (int i=0;i<gif->gct.size;i++) {
      const uint8_t r = gif->gct.colors[i*3];
      const uint8_t g = gif->gct.colors[i*3+1];
      const uint8_t b = gif->gct.colors[i*3+2];      
      uint16_t rgb = ((r & 0b11111000) << 8) | ((g & 0b11111100) << 3) | (b >> 3);
      gif->gct.colors[i*2]   = rgb&0xff;
      gif->gct.colors[i*2+1] = rgb>>8;
    }
...
render_frame_rect(gd_GIF *gif, uint8_t *buffer)
{
    ...
    for (j = 0; j < gif->fh; j++) {
        for (k = 0; k < gif->fw; k++) {
            ...
            color = &gif->palette->colors[index*2]; // xfm 3->2
            if (!gif->gce.transparency || index != gif->gce.tindex)
                memcpy(&buffer[(i+k)*2], color, 2); // xfm: 3->2
        }
        i += gif->width;
    }
}
